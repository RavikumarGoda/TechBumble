...
const SwipeInterface = ({ onShowProfile, sessionState, onSessionStateChange }: SwipeInterfaceProps) => {
  ...
  const { generateExplanation, generateAllQuestions } = useGeminiAI();
...
console.log("🧪 Current Filters:", filters);
  ...
  const generateAIQuestions = async (selections: FilterSelections, isRegeneration: boolean = false) => {
    ...
    try {
      ...
      // Make ONE SINGLE API CALL to avoid Gemini rate limits (15 RPM)
      const questionsBatch = await generateAllQuestions(
      ...
      for (const q of questionsBatch) {
        if (q.title && (isRegeneration || !allTimeUsedQuestionTitles.has(q.title))) {
          generatedQuestions.push({
            ...
            category: (q.category as any) || selections.categories[0] || 'DSA',
            difficulty: (q.difficulty as any) || selections.difficulties[0] || 'Medium',
