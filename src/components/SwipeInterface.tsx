...
const SwipeInterface = ({ onShowProfile, sessionState, onSessionStateChange }: SwipeInterfaceProps) => {
  ...
  const { generateExplanation, generateQuestionsBatch } = useGeminiAI();
...
console.log("🧪 Current Filters:", filters);
  ...
  const generateAIQuestions = async (selections: FilterSelections, isRegeneration: boolean = false) => {
    ...
    try {
      ...
      const totalCombinations = selections.categories.length * selections.difficulties.length;
      // We want ~15 questions total, fetch enough per combination, minimum 3.
      const countPerCombo = Math.max(3, Math.ceil(15 / totalCombinations));
      ...
      for (const category of selections.categories) {
        for (const difficulty of selections.difficulties) {
          fetchPromises.push(
            (async () => {
              try {
                const questionsBatch = await generateQuestionsBatch(category, difficulty, countPerCombo);
                const newQuestions: Question[] = [];
                for (const q of questionsBatch) {
                  if (q.title && (isRegeneration || !allTimeUsedQuestionTitles.has(q.title))) {
                    newQuestions.push({
                      id: `ai-${Date.now()}-${generatedQuestions.length}-${Math.random()}`,
                      title: q.title,
                      description: q.description || 'Practice question generated by AI',
                    ...
                    if (!isRegeneration) {
                      setAllTimeUsedQuestionTitles(prev => new Set([...prev, q.title]));
                    }
                  }
                }
                ...
                if (newQuestions.length > 0) {
                  ...
                  // Unblock UI immediately when the first batch arrives
                  if (!isFirstBatchLoaded) {
                    ...
                    if (isRegeneration) {
                      ...
                      toast({
                        ...
                        description: `Session started. Loading more questions in background...`
                      });
                    }
                  } else {
                    // Silently append remaining questions as they arrive
                    setQuestions(prev => [...prev, ...newQuestions].slice(0, 20));
                  }
                }
              } catch (error) {
                console.error(`Error generating questions for ${category} - ${difficulty}:`, error);
              }
            })()
          );
        }
