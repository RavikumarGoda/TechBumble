...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (companies) {
      ...
      if (comps.length > 0) where.companies = { hasSome: comps };
    }
    ...
    const questions = await prisma.question.findMany({
      where, take, skip,
      select: { id: true, title: true, description: true, category: true, difficulty: true, companies: true, explanation: true, solution: true },
    });
